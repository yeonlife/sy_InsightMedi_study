...
class MyWindow(QMainWindow):
    ...
    def set_current_frame(self):
        self.ds
        current_frame = self.dicom.pixel_array[self.current_frame_num]    #dicom 파일 불러온 변수로 dicom -> 바꿔주기
        pixmap = self.create_pixmap(current_frame)
        self.label.setPixmap(pixmap)
        
    def create_pixmap(self, current_frame):
        height, width = current_frame.shape
        

    def slider_value_changed(self, selected_frame):
        # slider command 구현
